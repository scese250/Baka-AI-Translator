...
class MangaOCREngine(OCREngine):
    ...
    def process_image(self, img: np.ndarray, blk_list: list[TextBlock]) -> list[TextBlock]:
        # Ensure model is on the correct device if it was unloaded
        # We need to rely on the previously configured device or default to cpu if unloaded
        # However, self.device was set to 'cpu' in unload(). 
        # But initialize() sets self.device. If we are reusing the engine, we might be stuck in CPU.
        # We need to restore it? 
        # Typically the factory or processor calls initialize() before process().
        # Let's check if we can restore it based on parameters or if initialize is called.
        
        # In OCRProcessor.process(), it calls OCRFactory.create_engine().
        # If caching is used, initialize is called ONCE.
        # If we set device='cpu' in unload, we lose the original preference.
        # We should store desired_device separately or rely on initialize being called again?
        # Actually, let's keep self.device as the *current* device, but check if we need to move it back?
        # No, if self.device is 'cpu', we run on 'cpu'.
        # If we want GPU, the caller should have called initialize('cuda').
        
        # If typical usage is:
        # engine = factory.get()
        # engine.initialize('cuda')
        # engine.process()
        # engine.unload() -> device='cpu'
        # Next time:
        # engine.initialize('cuda') -> device='cuda', move model to cuda.
        
        # So we just need to ensure initialize() moves it back.
