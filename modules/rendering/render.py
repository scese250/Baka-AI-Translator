...
def pyside_word_wrap(text: str, font_input: str, roi_width: int, roi_height: int,
    ...
    # Create a reusable QTextDocument and setup
    doc = QTextDocument()
    ...
    def eval_metrics(txt: str, font_sz: float) -> Tuple[float, float]:
        """Quick helper function to calculate width/height of text using reusable QTextDocument."""
        
        # Update settings on existing doc
    ...
    while lo <= hi:
        ...
        if w <= roi_width and h <= roi_height:
            found_fit = True
            best_text, best_size, best_h = wrapped, mid, h
            lo = mid + 1
        else:
            hi = mid - 1
    ...
    return best_text, best_size, best_h
...
def manual_wrap(main_page, blk_list: List[TextBlock], font_family: str, line_spacing, 
    ...
    for blk in blk_list:
        ...
        translation, font_size, text_height = pyside_word_wrap(translation, font_family, width, height,
                                                 line_spacing, outline_width, bold, italic, underline,
                                                 alignment, direction, init_font_size, min_font_size)
        
        main_page.blk_rendered.emit(translation, font_size, text_height, blk)
