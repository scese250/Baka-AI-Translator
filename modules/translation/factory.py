...
from .llm.ai_studio import AIStudioTranslation
...
class TranslationFactory:
    ...
    @classmethod
    def create_engine_uncached(cls, settings, source_lang: str, target_lang: str, translator_key: str) -> TranslationEngine:
        """
        Create a fresh engine instance, bypassing cache.
        Used for parallel batch processing where each thread needs its own engine.
        """
        engine_class = cls._get_engine_class(translator_key)
        engine = engine_class()
        if translator_key in cls.TRADITIONAL_ENGINES:
