...
def get_providers(device: Optional[str] = None) -> list[Any]:
    ...
    configured: list[Any] = []
    for p in available:
        # User requested to avoid TensorRT due to missing DLLs with GTX 1060
        # UPDATE: TensorRT 10 does NOT support Pascal (SM 6.1/GTX 1060).
        # We must disable it to avoid "Target GPU SM 61 is not supported" error.
