...
class TextBlockItem(QGraphicsTextItem):
    ...
    def set_text_class(self, text_class):
        self.text_class = text_class
    ...
    def paint(   
        ...
        if self.selection_outlines:
            ...
            for outline_info in self.selection_outlines:
                ...
                fmt = cursor.charFormat()
                c = outline_info.color if outline_info.color else QColor(255, 255, 255)
                fmt.setForeground(c)
                cursor.mergeCharFormat(fmt)

                # Draw the outline using a filled-disc of offsets
                # This covers all integer pixel positions within the radius
                # to produce a continuous stroke at any outline width
                w = outline_info.width
                r = max(1, int(math.ceil(w)))
                r_sq = w * w
                offsets = [
                    (dx, dy)
    ...
    def get_selected_text_properties(self, cursor: QTextCursor):
        ...
        if containing_outlines:
            ...
            outline_properties = {
                'outline': True,
                'outline_color': latest_outline.color.name() if latest_outline.color else None,
