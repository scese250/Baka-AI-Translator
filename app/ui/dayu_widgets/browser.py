    ...
    def _get_valid_file_list(self, url_list):
        ...
        for url in url_list:
            ...
            if os.path.exists(file_name):
                # If it's a directory, accept it (checking content usually happens later or we process the dir itself)
                if os.path.isdir(file_name):
                    file_list.append(file_name)
                # If it's a file, check filters
                elif os.path.isfile(file_name):
                    if self.get_dayu_filters():
                        if os.path.splitext(file_name)[-1].lower() in [f.lower() for f in self.get_dayu_filters()]:
                            file_list.append(file_name)
                    else:
                        file_list.append(file_name)
