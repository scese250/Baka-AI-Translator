...
class TextController:
    ...
    def on_blk_rendered(self, text: str, font_size: int, text_height: int, blk: TextBlock):
        ...
        if hasattr(render_settings, 'color_overrides') and render_settings.color_overrides:
            blk_class = getattr(blk, 'text_class', 'text_bubble') # Default to text_bubble if not set
            ...
            if class_settings:
                ...
                if override_text_color:
                     text_color = QColor(override_text_color)
                ...
                if class_settings.get('outline_enabled', False):
                    ...
                    # If global outline is enabled but class outline is NOT, should we disable it?
                    # The request implies specific configuration per bubble. 
                    # If I explicitly configure a bubble type, it should probably override global settings.
                    # But if the global "Outline" checkbox is what enables outlines in general?
                    # The UI I added has a per-class "Outline" checkbox.
                    # So if that is unchecked, this class should NOT have an outline, even if global is checked?
                    # Or does it override only if we want to FORCE an outline?
                    # Let's assume per-class settings take precedence if present.
                    # If class settings are present (which they always are due to initialization),
                    # we use the class's outline enabled state?
                    # Actually, let's treat the global settings as defaults, and class settings as overrides.
                    # BUT my UI creates a full configuration for each class.
                    # So if I use the class config, I should probably use it fully.
                    # However, currently the global settings are what's visible in the main toolbar.
                    # The user requested "configure colors for different bubbles".
                    # Let's stick to using the override if the Outline checkbox IN THE SETTINGS is checked/unchecked.
                    
                    # Wait, my logic above: 
                    # if class_settings.get('outline_enabled', False): -> Apply outline settings
                    # else: -> What? 
                    # If I uncheck "Outline" for "Bubble Text", I expect no outline for bubbles.
        ...
        properties = TextItemProperties(
            ...
            underline=underline,
            direction=direction,
            position=(blk.xyxy[0], blk.xyxy[1] + y_offset),
            rotation=blk.angle,
        )
    ...
    def on_text_class_change(self, display_name: str):
        if self.main.curr_tblock_item and display_name:
            # Map display name to internal name
            text_class = self.main.class_mapping.get(display_name, "text_bubble")
            
            old_item = copy.copy(self.main.curr_tblock_item)
            self.main.curr_tblock_item.set_text_class(text_class)

            # Update the underlying TextBlock in blk_list if it exists
            if self.main.curr_tblock:
                self.main.curr_tblock.text_class = text_class
                
            # Apply styling for the new class
            render_settings = self.render_settings()
            if hasattr(render_settings, 'color_overrides') and render_settings.color_overrides:
                class_settings = render_settings.color_overrides.get(text_class)
                if class_settings:
                    # Text Color
                    override_text_color = class_settings.get('text_color')
                    if override_text_color:
                        color = QColor(override_text_color)
                        self.main.curr_tblock_item.set_color(color)
                        # Update the color button to reflect change
                        ...
                        # Fallback to smart color / detection if no override
                        # We might need to restore original color if we are toggling back
                        # Ideally, set_text_class shouldn't apply invalid colors
                        pass

                    # Outline
                    if class_settings.get('outline_enabled', False):
                        override_outline_color = class_settings.get('outline_color')
                        outline_color = QColor(override_outline_color) if override_outline_color else None
                        
                        override_outline_width = class_settings.get('outline_width')
                        outline_width = float(override_outline_width) if override_outline_width else 1.0
                        ...
                        self.main.outline_width_dropdown.setCurrentText(str(outline_width))
                        self.main.outline_checkbox.setChecked(True)
                    else:
                        # Disable outline for this class?
                         # Or just don't apply anything and leave as is?
                         # It's cleaner to apply strict class settings.
                         # If class says NO outline, we should probably disable it.
    ...
    def set_values_for_blk_item(self, text_item: TextBlockItem):
        ...
        try:
            ...
            # Set Class Dropdown
            # Find display name for the text_class
            text_class = getattr(text_item, 'text_class', 'text_bubble')
            display_name = next((k for k, v in self.main.class_mapping.items() if v == text_class), None)
            if display_name:
                self.main.text_class_dropdown.setCurrentText(display_name)
    ...
    def render_settings(self) -> TextRenderingSettings:
        ...
        return TextRenderingSettings(
            ...
            direction = direction,
            color_overrides = self.main.settings_page.ui.text_rendering_page.get_settings().get('color_overrides', {})
        )