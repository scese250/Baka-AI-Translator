...
class ProjectController:
    ...
    def save_and_make_worker(self, output_path: str):
        ...
        _export_settings = self.main.settings_page.get_export_settings()
        ...
        try:            
            if self.main.webtoon_mode:
                ...
                for page_idx, file_path in enumerate(self.main.image_files):
                    ...
                    sv_pth = os.path.join(temp_dir, f"{bname_no_ext}{_image_ext}")
    ...
    def load_state_to_ui(self, llm_settings: dict):
        if isinstance(llm_settings, str):
             # Backward compatibility if something goes wrong or old cached logic
            self.main.settings_page.ui.extra_context.setPlainText(llm_settings)
        else:
            self.main.settings_page.ui.extra_context.setPlainText(llm_settings.get('extra_context', ''))
            self.main.settings_page.ui.system_prompt_enabled.setChecked(llm_settings.get('system_prompt_enabled', False))
            self.main.settings_page.ui.system_prompt.setPlainText(llm_settings.get('system_prompt', ''))
    ...
    def load_main_page_settings(self):
        ...
        source_lang = settings.value("source_language", "Japanese")
        target_lang = settings.value("target_language", "English")

        # Use reverse mapping to get the translated language names
        self.main.s_combo.setCurrentText(self.main.reverse_lang_mapping.get(source_lang, self.main.tr("Japanese")))
    ...
    def process_group(self, group_key, group_value, settings_obj):
