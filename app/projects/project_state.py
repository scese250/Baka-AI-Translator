...
def save_state_to_proj_file(comic_translate: ComicTranslate, file_name: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        state = {
            ...
            'llm_system_prompt_enabled': comic_translate.settings_page.get_llm_settings().get('system_prompt_enabled', False),
...
def load_state_from_proj_file(comic_translate: ComicTranslate, file_name: str):
    ...
    # Inject original_path into the state so we can recover folder names export
    comic_translate.image_states = {}
    for file, img_state in image_states.items():
        temp_path = original_to_temp.get(file, file)
        img_state['original_path'] = file
        comic_translate.image_states[temp_path] = img_state
    ...
    # restore LLM extra context
    return {
        'extra_context': state.get('llm_extra_context', ''),
        'system_prompt_enabled': state.get('llm_system_prompt_enabled', False),
