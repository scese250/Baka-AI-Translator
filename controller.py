...
class ComicTranslate(ComicTranslateUI):
    ...
    def handle_dropped_files(self, files: list[str]):
        ...
        if len(files) == 1 and files[0].lower().endswith('.ctpr'):
            if self._confirm_start_new_project():
                self.project_ctrl.thread_load_project(files[0])
            return

        if self.image_files:
            self.image_ctrl.thread_insert(files)
        else:
            self._guarded_thread_load_images(files)
    ...
    def connect_ui_elements(self):
        ...
        # Sync language combos: main ↔ settings
        def _sync_src_to_settings(text):
            w = self.settings_page.ui.source_lang_combo
            w.blockSignals(True)
            w.setCurrentText(text)
            w.blockSignals(False)
        def _sync_src_to_main(text):
            w = self.s_combo
            w.blockSignals(True)
            w.setCurrentText(text)
            w.blockSignals(False)
            self.text_ctrl.save_src_trg()
        def _sync_tgt_to_settings(text):
            w = self.settings_page.ui.target_lang_combo
            w.blockSignals(True)
            w.setCurrentText(text)
            w.blockSignals(False)
        def _sync_tgt_to_main(text):
            w = self.t_combo
            w.blockSignals(True)
            w.setCurrentText(text)
            w.blockSignals(False)
            self.text_ctrl.save_src_trg()
        self.s_combo.currentTextChanged.connect(_sync_src_to_settings)
        self.settings_page.ui.source_lang_combo.currentTextChanged.connect(_sync_src_to_main)
        self.t_combo.currentTextChanged.connect(_sync_tgt_to_settings)
        self.settings_page.ui.target_lang_combo.currentTextChanged.connect(_sync_tgt_to_main)
        ...
        self.image_viewer.files_dropped.connect(self.handle_dropped_files)

        # Rendering
        self.font_dropdown.currentTextChanged.connect(self.text_ctrl.on_font_dropdown_change)
        # Sync font selectors: main ↔ settings
        def _sync_font_to_settings(font_text):
            from PySide6.QtGui import QFont
            sel = self.settings_page.ui.font_selector
            sel.blockSignals(True)
            sel.setCurrentFont(QFont(font_text))
            sel.blockSignals(False)
        def _sync_font_to_main(font_text):
            from PySide6.QtGui import QFont
            dd = self.font_dropdown
            dd.blockSignals(True)
            dd.setCurrentFont(QFont(font_text))
            dd.blockSignals(False)
            self.text_ctrl.on_font_dropdown_change(font_text)
        self.font_dropdown.currentTextChanged.connect(_sync_font_to_settings)
        self.settings_page.ui.font_selector.currentTextChanged.connect(_sync_font_to_main)
        self.text_class_dropdown.currentTextChanged.connect(self.text_ctrl.on_text_class_change)
    ...
    def start_batch_process(self):
        ...
        # Check for existing output files (Resume/Restart logic)
        paths_to_process = self.image_files
        # Currently only supported for standard BatchProcessor (not Webtoon mode explicitly checked here, 
        # but if names match it might work. Restricting logic to standard mode for safety or applying generally if compatible).
        # We will apply generally but if check returns empty, no prompt appears.
        if not self.webtoon_mode:
            existing_files = self.pipeline.batch_processor.check_existing_output(self.image_files)
            if existing_files:
                msg = QMessageBox(self)
                msg.setWindowTitle(self.tr("Existing Files Detected"))
                msg.setText(self.tr(f"Found {len(existing_files)} images that already have a translated version."))
                msg.setInformativeText(self.tr("Do you want to Resume (skip existing) or Restart (overwrite all)?"))
                resume_btn = msg.addButton(self.tr("Resume"), QMessageBox.YesRole)
                restart_btn = msg.addButton(self.tr("Restart"), QMessageBox.NoRole)
                ...
                if msg.clickedButton() == cancel_btn:
                    ...
                    paths_to_process = [p for p in self.image_files if p not in existing_files]
                    if not paths_to_process:
                        Messages.show_info(self, self.tr("Info"), self.tr("All files skipped."))
                        return
        ...
        if self.webtoon_mode:
            # TODO: Add global render settings to webtoon batch process too if needed, similar logic
            self.run_threaded(self.pipeline.webtoon_batch_process, None, self.default_error_handler, self.on_batch_process_finished)
        else:
            # Capture Settings in MAIN THREAD (Safe Access)
            r_settings = self.render_settings()
            # Pass explicit paths in case we filtered them
            self.run_threaded(lambda: self.pipeline.batch_process(paths_to_process, render_settings=r_settings), None, self.default_error_handler, self.on_batch_process_finished)
    ...
    def batch_translate_selected(self, selected_file_names: list[str]):
        ...
        # Check for existing output files (Resume/Restart logic)
        if not self.webtoon_mode:
            existing_files = self.pipeline.batch_processor.check_existing_output(selected_paths)
            if existing_files:
                msg = QMessageBox(self)
                msg.setWindowTitle(self.tr("Existing Files Detected"))
                msg.setText(self.tr(f"Found {len(existing_files)} images that already have a translated version."))
                ...
                if msg.clickedButton() == cancel_btn:
                    ...
                    paths_to_process = [p for p in selected_paths if p not in existing_files]
                    if not paths_to_process:
                        Messages.show_info(self, self.tr("Info"), self.tr("All files skipped."))
                        return
                    self.selected_batch = paths_to_process
        ...
        if self.webtoon_mode:
            ...
            self.run_threaded(
                lambda: self.pipeline.webtoon_batch_process(paths_to_process),
                None,
            ...
            self.run_threaded(
                lambda: self.pipeline.batch_process(paths_to_process, render_settings=r_settings),
                None,
                self.default_error_handler,
                self.on_batch_process_finished
            )
    ...
    def update_translated_text_items(self, single_blk: bool):
        def set_new_text(text_item, wrapped, font_size, text_height, blk):
            ...
            # Recenter
            block_height = blk.xyxy[3] - blk.xyxy[1]
            y_offset = (block_height - text_height) / 2
            new_y = blk.xyxy[1] + y_offset
            
            # Update Y position only, keep X as is (or reset to blk.x?)
            # Usually X is blk.x because alignment is internal
            text_item.setPos(text_item.pos().x(), new_y)
            ...
            for text_item in text_items_to_process:
                ...
                blk = next(
                    (
                        ...
                        # Check strict X match, looser Y match (must be roughly within block vertical range)
                        if is_close(b.xyxy[0], x1, 5) and 
                        (b.xyxy[1] - 10 <= y1 <= b.xyxy[3] + 10) and
                ...
                if hasattr(rs, 'color_overrides') and rs.color_overrides:
                    blk_class = getattr(blk, 'text_class', 'text_bubble')
                    class_settings = rs.color_overrides.get(blk_class)
                    
                    if class_settings:
                        # Text Color
                        override_text_color = class_settings.get('text_color')
                        if override_text_color:
                             text_item.set_color(QColor(override_text_color))
                        
                        # Outline
                        if class_settings.get('outline_enabled', False):
                            override_outline_color = class_settings.get('outline_color')
                            out_color = QColor(override_outline_color) if override_outline_color else None
                            
                            override_outline_width = class_settings.get('outline_width')
                            out_width = float(override_outline_width) if override_outline_width else 1.0
                            
                            text_item.set_outline(out_color, out_width)
                ...
                self.run_threaded(
                    pyside_word_wrap,
                    lambda wrap_res, ti=text_item, b=blk: set_new_text(ti, wrap_res[0], wrap_res[1], wrap_res[2], b),
                    self.default_error_handler,
                    None,
                    *wrap_args
                )
