...
class WebtoonBatchProcessor:
    ...
    def __init__(
        ...
        # Translator reuse for maintaining chat context (important for Gemini)
    ...
    def _process_virtual_chunk(self, vpage1: VirtualPage, vpage2: VirtualPage,
        ...
        if blk_list:
            ...
            if self.translator is None or source_lang != self.last_source_lang or target_lang != self.last_target_lang:
                self.translator = Translator(self.main_page, source_lang, target_lang)
                self.last_source_lang = source_lang
                self.last_target_lang = target_lang
                logger.info(f"Initialized translator for {source_lang} -> {target_lang}")
            
            try:
                self.translator.translate(blk_list, combined_image, extra_context)
            except Exception as e:
                if isinstance(e, requests.exceptions.HTTPError):
                    try:
                        err_msg = e.response.json().get("error_description", str(e))
    ...
    def _emit_and_store_virtual_page_results(self, vpage: VirtualPage, blk_list_virtual: List[TextBlock]):
        ...
        for blk_virtual in blk_list_virtual:
            ...
            # Apply Class Overrides (per text_class color configuration)
            if hasattr(render_settings, 'color_overrides') and render_settings.color_overrides:
                blk_class = getattr(blk_virtual, 'text_class', 'text_bubble')
                ...
                if class_settings:
                    ...
                    if class_settings.get('outline_enabled', False):
                        ...
                        # If outline is disabled for this class, force no outline
                        outline_color = None
            ...
            if should_emit_live:
                self.main_page.blk_rendered.emit(translation, font_size, text_height, render_blk)
    ...
    def _save_final_rendered_page(self, page_idx: int, image_path: str, timestamp: str):
        ...
        extension = getattr(self, '_image_ext', os.path.splitext(image_path)[1])
        ...
        _quality = getattr(self, '_image_quality', 100)
        renderer.save_image(sv_pth, _quality)
    ...
    def webtoon_batch_process(self, selected_paths: List[str] = None):
        ...
        _export_settings = self.main_page.settings_page.get_export_settings()
        self._image_format = _export_settings.get('image_format', 'PNG').lower()
        self._image_ext = f'.{self._image_format}'
        self._image_quality = _export_settings.get('image_quality', 100)
